...
user=$(logname)
profile=$(find /home/$user/.mozilla/firefox/*.default-release | grep /home/$user/.mozilla/firefox/*.default-release | cut -d'/' -f6- | cut -d/ -f1 | sort | uniq)


# set global extensions installation path
# you may have to adapt it to your environment
PATH_FIREFOX="/home/$user/.mozilla/firefox/$profile/extensions"
...
then
  ...
  # set root ownership
  chown -R $user:$user "$PATH_EXTENSION/$UID_ADDON"
